...
const Header = () => {
  ...
  const cartItems = useSelector((store) => store.cart.items); // Subscribing to a store using selector
  ...
  return (
    <header className="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-white shadow-md">
      ...
      <nav className="flex gap-6 mt-4 md:mt-0 items-center">
        ...
        <Link className="text-gray-600 hover:text-purple-600 transition" to="/cart">
